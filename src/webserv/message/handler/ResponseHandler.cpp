...
void ResponseHandler::setResponseStatusLine() {
  ...
  response_->getHeaderMsg() += to_string(this->response_->getStatusCode());
  response_->getHeaderMsg() += " ";
...
void ResponseHandler::setErrorBody(const std::string &error_page_directory_path) {
  ...
  if (error_page_directory_path != "") {
    std::string error_page_path = error_page_directory_path + "/" + to_string(response_->getStatusCode()) + ".html";
...
void ResponseHandler::setDefaultErrorBody() {
  ...
  body_buf_->append("<head><title>" + to_string(response_->getStatusCode()) + " " + StatusMessage::of(response_->getStatusCode()) + "</title></head>\r\n");
  body_buf_->append("<body>\r\n");
  body_buf_->append("<center><h1>" + to_string(response_->getStatusCode()) + " " + StatusMessage::of(response_->getStatusCode()) + "</h1></center>\r\n");
...
void ResponseHandler::setAutoindexBody(const std::string &uri, const std::string &filepath) {
  ...
  while ((item = readdir(dir_ptr))) {
    ...
    std::string filesize = (S_ISDIR(this->stat_buffer_.st_mode) ? "-" : to_string(this->stat_buffer_.st_size));
...
void ResponseHandler::createLocationHeaderFor201(Connection *c, Request &request) {
  ...
  if (request.getPort().empty())
    request.setPort(to_string(htons(c->getSockaddrToConnect().sin_port)));
